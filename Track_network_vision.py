...
#################################################################################
license_folder = '/home/huaxin/data_3T/license_dataset/license_track/dataset_0/'
...
# annotations_files = sorted(glob.glob(os.path.join(license_folder, 'bbox_y_gt')))
print image_files
...
for num_img in range(0,len(image_files)):
    ...
    print '\ny_temp_float_cur',y_temp_float_cur

    if y_temp_float_cur[0]!=num_img:
        print '\ndataset read error: index not match\n'
        cv2.waitKey(0)

    bbox_curr_y = [y_temp_float_cur[1],y_temp_float_cur[2],y_temp_float_cur[3],y_temp_float_cur[4]]
    ...
    for epoch in range(epochs):
        ...
        for batch in range(len(train_x_path_curr)//batch_size):
            ...
            if (epoch+1)%50 == 0 and batch ==1:
                ...
                save_path = saver.save(sess, "./tmp/model_vision_"+time+".ckpt") #
